...
parameter integer IMEM_UNIT = 2**12;  // in line with cpu imem ADDR_W=9
...
integer counter, instr_mem_counter;

function automatic integer count_file_lines(string filename);
   integer fd;
   string line;
   integer num_lines;
   begin
         ...
         while (!$feof(fd)) begin
            void'($fgets(line, fd));   // read one line  :contentReference[oaicite:3]{index=3}
            num_lines++;
         end
         $fclose(fd);
         return num_lines;
...
begin
   ...
   instr_mem_counter = count_file_lines("../SIM/data/imem_content.txt");
   $readmemh("../SIM/data/imem_content.txt",instr_mem);
   cnt_and_wait(10);
   
   for (imem_cnt = 0; imem_cnt < instr_mem_counter+1; imem_cnt = imem_cnt+1)  begin
      wait(clk==1'b0);
      wen_ext   = 1'b1;
      ren_ext   = 1'b0;
